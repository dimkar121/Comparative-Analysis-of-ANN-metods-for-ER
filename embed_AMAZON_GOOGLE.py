...
if __name__ == '__main__':
    ...
    df = pd.read_csv("./data/Amazon.csv", sep=",", encoding="unicode_escape")
    embed(
        df=df,
        text_columns=["title","description","manufacturer","price"],
        prefix="",
        output_filename=f'{folder}/Amazon_{model_tag}.pqt',
        model=embedding_model 
    )

    df = pd.read_csv("./data/GoogleProducts.csv", sep=",", encoding="unicode_escape")
    embed(
        df=df,
        text_columns=["name","description","manufacturer","price"],
        prefix="",
        output_filename=f'{folder}/Google_{model_tag}.pqt',
        model=embedding_model
    )
