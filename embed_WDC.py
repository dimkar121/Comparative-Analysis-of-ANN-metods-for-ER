...
if __name__ == '__main__':
    ...
    df = pd.read_csv("./data/WDCA.csv", sep=",", encoding="utf-8")
    embed(
        df=df,
        text_columns=["brand","title","description","price","cluster_id","pricecurrency","page_url"],
        prefix="",
        output_filename=f'{folder}/WDCA_{model_tag}.pqt',
        model=embedding_model 
    )

    df = pd.read_csv("./data/WDCB.csv", sep=",", encoding="utf-8")
    embed(
        df=df,
        text_columns=["brand","title","description","price","cluster_id","pricecurrency","page_url"],
        prefix="",
        output_filename=f'{folder}/WDCB_{model_tag}.pqt',
        model=embedding_model
    )
