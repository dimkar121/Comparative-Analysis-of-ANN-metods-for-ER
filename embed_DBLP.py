...
if __name__ == '__main__':
    ...
    df = pd.read_csv("./data/test_dblp_A.txt", sep=",", encoding="utf-8")
    embed(
        df=df,
        text_columns=["author1","author2","title","year"],
        prefix="",
        output_filename=f'{folder}/test_dblp_A_{model_tag}.pqt',
        model=embedding_model 
    )

    df = pd.read_csv("./data/test_dblp_B.txt", sep=",", encoding="utf-8")
    embed(
        df=df,
        text_columns=["author1","author2","title","year"],
        prefix="",
        output_filename=f'{folder}/test_dblp_B_{model_tag}.pqt',
        model=embedding_model
    )
