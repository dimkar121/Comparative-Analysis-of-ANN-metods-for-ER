...
if __name__ == '__main__':
    ...
    df = pd.read_csv("./data/test_voters_A.txt", sep=",", encoding="unicode_escape")
    embed(
        df=df,
        text_columns=["surname","name","address","town","ps"],
        prefix="",
        output_filename=f'{folder}/test_voters_A_{model_tag}.pqt',
        model=embedding_model 
    )

    df = pd.read_csv("./data/test_voters_B.txt", sep=",", encoding="unicode_escape")
    embed(
        df=df,
        text_columns=["surname","name","address","town","ps"],
        prefix="",
        output_filename=f'{folder}/test_voters_B_{model_tag}.pqt',
        model=embedding_model
    )
